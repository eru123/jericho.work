...
const cwd = cwd + '/client';
gulp.task('cdn-install', function (done) {
  spawn('pnpm', ['install'], { cwd: cwd + '/client/cdn' }).on('close', done);
});

gulp.task('admin-install', function (done) {
  spawn('pnpm', ['install'], { cwd: cwd + '/client/admin' }).on('close', done);
});

gulp.task('cdn-dev', function (done) {
  spawn('pnpm', ['run', 'dev'], { cwd: cwd + '/client/cdn' }).on('close', done);
});

gulp.task('admin-dev', function (done) {
  spawn('pnpm', ['run', 'dev'], { cwd: cwd + '/client/admin' }).on('close', done);
});

gulp.task('cdn-build', function (done) {
  spawn('pnpm', ['run', 'build'], { cwd: cwd + '/client/cdn' }).on('close', done);
});

gulp.task('admin-build', function (done) {
  spawn('pnpm', ['run', 'build'], { cwd: cwd + '/client/admin' }).on('close', done);
});

gulp.task('cf_cache_purge', function (done) {
  spawn('/bin/php', ['cf_cache_purge.php'], { cwd: cwd + '/scripts' }).on('close', done);
});
